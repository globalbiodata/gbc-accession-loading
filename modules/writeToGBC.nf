process WRITE_TO_GBC {
    ...
    input:
    tuple val(meta), path(json_file), path(accession_types), val(db), path(db_creds), path(version_json)
    ...
    --summary ${summary_file} --db ${db} --dbcreds ${db_creds} --version ${version_json}
    """
}